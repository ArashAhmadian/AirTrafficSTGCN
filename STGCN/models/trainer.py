...
def model_train(inputs, blocks, args, sum_path='./output/tensorboard'):
    ...
    # Define model loss
    train_loss,valid_loss, pred = build_model(x, n_his, Ks, Kt, blocks, keep_prob)
     
    train_summ=[]
    valid_summ=[]

    train_summ.append(tf.summary.scalar('RMSE Training Loss', train_loss[0]))
    train_summ.append(tf.summary.scalar('L2 Norm Training Loss', train_loss[1]))

    valid_summ.append(tf.summary.scalar('RMSE Validation Loss', valid_loss[0]))
    valid_summ.append(tf.summary.scalar('L2 Norm Validation Loss', valid_loss[1]))

    copy_loss = tf.add_n(tf.get_collection('copy_loss'))
    copy_summ=tf.summary.scalar('copy_loss', copy_loss)
    ...
    with tf.control_dependencies([step_op]):
        if opt == 'RMSProp':
            train_op = tf.train.RMSPropOptimizer(lr).minimize(train_loss[1])
        elif opt == 'ADAM':
            train_op = tf.train.AdamOptimizer(lr).minimize(train_loss[1])
        else:
            raise ValueError(f'ERROR: optimizer "{opt}" is not defined.')

    merged = tf.summary.merge([train_summ, copy_summ])
    merged_2 = tf.summary.merge([valid_summ])
    ...
    with tf.Session() as sess:
        ...
        for i in range(epoch):
            ...
            for j, x_batch in enumerate(
                ...
                if (j in range(0,batch_size,4)):
                    loss_value = \
                        sess.run([train_loss[1], copy_loss],
                                    feed_dict={x: x_batch[:, 0:n_his + 1, :, :], keep_prob: 1.0})    
                    print(f'Epoch {i:2d}, Step {j:3d}: [{loss_value[0]:.3f}, {loss_value[1]:.3f}]')

                for k, val_batch in enumerate(
                        gen_batch(inputs.get_data('val'), dynamic_batch=True, shuffle=True)):
                    summary_2 = sess.run([merged_2], feed_dict={x: val_batch[:, 0:n_his + 1, :, :], keep_prob: 1.0})
                    writer.add_summary(summary_2[0], i * epoch_step + j)
                    #writer.add_summary(summary_2[1], i * epoch_step + j)    
