...
def seq_gen(len_seq, data_seq, offset, n_frame, n_route, day_slot=1, C_0=1):
    '''
    ...
    tmp_seq = np.zeros((length * n_slot, n_frame, n_route, C_0))
...
def data_gen(file_path, n_route, n_frame=21, day_slot=288):
    ...
    total_points = int((len(data_seq) - n_frame) / day_slot)
    n_train = int(round(total_points * 0.7))
    n_val = int(round(total_points *0.15))
    n_test  = total_points - n_train - n_val 
...
def gen_batch(inputs, batch_size_=None, dynamic_batch=False, shuffle=False):
    '''
